...
//UART to communicate with RN42
#define BP_UART             BTSPP_UART
#define BP_UART_NBR          460800
#define BP_UART_BR          "460K"

#define BP_RESET            BTSPP_RST
...
#define BP_GET_FIRMWARE_VER "V\r\n"

//Bluetooth system commands
#define BP_REBOOT  				"R,1\r\n"
#define BP_RECONNECT  			"C\r\n"
#define BP_CHANGE_NAME			"SN,"
#define BP_RESET_TO_FACTORY_DEFAULT   "SF,1\r\n"
#define BP_TEMP_CHANGE_BR   "U," BP_UART_BR ",N\r\n"
...
public:
  BPLib();
  bool begin(char BP_Mode[], char BP_Type[]);
  bool sendCmd(char BP_CMD[]);
  void sendByte(uint8_t byte);
  void sendBuffer(uint8_t *buffer, uint16_t size);
  void sendString(char *string);
  uint8_t readRaw(uint8_t *buffer, uint16_t size);
  int available();
...
//  void gameJoyMove(signed int BP_X1, signed int BP_Y1, signed int BP_X2,
//      signed int BP_Y2);
//  void gameJoyReleaseAll();
  bool connected();
  bool changeName(char BP_NAME[]);
private:
  UART_Handle uart;
  UART_Params uartParams;

  bool get(char BP_STAT[], uint16_t strlen);
};
#endif
